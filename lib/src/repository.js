"use strict";

var redis = require("redis");
var async = require("async");
var logger = require('../src/logger.js');

var log = function _log(text, text2) {
    logger.logDebug('repository.js', null, text + ' ' + text2);
};


//TODO: move constants to a configuration place
var client = redis.createClient(14583, 'pub-redis-14583.eu-west-1-1.2.ec2.garantiadata.com');

client.on('error', function(err){
   log("redis error", err);
});


var saveUserData = function (userID, serializedExercises, callback) {
    if(Object.keys(serializedExercises).length > 0 ){
        client.hmset(userID, serializedExercises, callback);
    }else{
        client.del(userID, callback);
    }
};

var _counter = 0;
var _counterCallback = 0;

var loadUserData = function (userID, callback) {
    _counter++;
    var myFunc = function (err, b){
        _counterCallback++;
        log('_counterCallback', _counterCallback);
        callback(err, b);
    };
    log('loadUserData', _counter);
    client.hgetall(userID, myFunc);
};

/**
 * Saves exercise data to the repository.
 * @param userID
 * @param name
 * @param json
 * @param callback
 */
var saveExerciseData = function (userID, name, json, callback) {
//    log("saveExerciseData;");
    client.hset(userID, name, json, callback);
};

var loadExerciseData = function (userName, name, callback) {
    client.hget(userName, name, function (err, res) {
        if (err) {
            callback(err, res);
        }
        else {
            var val = {};
            try {
                val = JSON.parse(res);
            }
            catch (e) {
                err = e.message;
            }
            finally {
                callback(err, val);
            }
        }
    });
};

var close = function () {
    client.quit();
};

var deleteUserData = function (userID, callback) {
    client.del(userID, callback);
};

var getAllKeys = function (callback) {
    client.keys("*", callback);
};


module.exports.getAllKeys = getAllKeys;
module.exports.deleteUserData = deleteUserData;
module.exports.loadExerciseData = loadExerciseData;
module.exports.saveExerciseData = saveExerciseData;
module.exports.loadUserData = loadUserData;
module.exports.saveUserData = saveUserData;
module.exports.close = close;

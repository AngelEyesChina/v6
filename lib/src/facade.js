...
facade.tree.renderTree = function _renderTree(userID, momentDate, callback) {
    var myTree = tree(userID);
    myTree.loadTree(function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null, myTree.render(null, null, momentDate));
        }
    });
};
...
facade.exercise.getExerciseListing = function _getExerciseListing(userID, exerciseID, momentDate, callback) {
    var myTree = tree(userID);
    myTree.loadTree(function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null, myTree.render(exerciseID, momentDate));
        }
    });
};

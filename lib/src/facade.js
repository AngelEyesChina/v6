...
facade.tree.renderTree = function _renderTree(userID, momentDate, callback) {
    ...
    myTree.loadTree(function (err) {
        if (err) {
            ...
            callback(myTree.render(null, null, momentDate));
        }
    });
};
...
facade.exercise.getExerciseListing = function _getExerciseListing(userID, exerciseID, callback) {
    var myTree = tree(userID);
    myTree.loadTree(function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null, myTree.render(exerciseID));
        }
    });
};

...
//        log("tearDown;");
  ...
  testMarkExercise: function (test) {
    ...
    async.waterfall([
      ...
      var expected = {'ID': ID, 'title': consts.TEST_EXERCISE_NAME, 'type': consts.TEST_EXERCISE_TYPE, 'parentID': null, displayOrder: 999, 'marked': true, 'lastNDates': [moment(consts.TEST_MARK_DATE).toISOString()], 'children': []};
  ...
  testComplexSaveAndLoad: function (test) {
    ...
    async.waterfall([
      ...
      test.equal(util.inspect(actual, { depth: null }), util.inspect(expected, { depth: null }), "testComplexSaveAndLoad - The marked exercise didn't render as expected");
      test.done();
    });
  },
